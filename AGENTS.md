...
- `requirements_unsloth.txt`: unsloth fine-tuning lab. Installed into a **separate** venv (`/tmp/.venv-unsloth`) with its own `NotoLab (Unsloth)` Jupyter kernel by `setup_adv.sh`, `setup_peft.sh`, and `setup_sllm.sh`, because unsloth pulls its own `transformers`/`trl` versions that conflict with the main environment. It pins the same `torch` as the main manifests so the heavy CUDA layer is hard-linked from the uv cache instead of re-downloaded.
...
- `release.sh`: instructor-side release tool. `bash release.sh <version>` tags the current `origin/main`, bakes that tag into `NOTOLAB_REF` in copies of the four setup scripts, and publishes them plus the four manifests and the four lock files as GitHub release assets. It does **not** generate locks: it consumes validated ones from `locks/` (gitignored) and refuses to publish if a manifest's `==` pins are missing from the matching lock, or if an asset failed to upload. Locks are produced and installation-tested in a GPU container first — see `TODO.md`.
- `setup_adv.sh`, `setup_peft.sh`, `setup_sllm.sh`, `setup_agent.sh`: Linux/NotoLab bootstrap scripts that create `/workspace/lab`, install `uv`, build `/tmp/.venv`, install dependencies, register the `NotoLab` Jupyter kernel, and write runtime guidance. Each script downloads its matching `requirements_*.txt` from the ref in its `NOTOLAB_REF` variable (default `main`) at install time, so manifest edits take effect only after they are pushed to `main`. `setup_adv.sh` installs neither Ollama nor llama.cpp (9 steps); `setup_peft.sh` / `setup_sllm.sh` install both (11 steps); `setup_agent.sh` installs Ollama and llama.cpp but no unsloth (10 steps).
