...
step 2 "APT 일괄 설치 (pciutils, chromium, Node.js, Playwright Chrome)"
# ⚠️ 이 스크립트가 직접 부르는 apt 작업은 전부 이 단계에 모은다.
#    끝나면 [APT_DONE] 토큰을 출력하므로, Claude Code 배치 설치(apt 사용)는
#    그 토큰을 보고 시작하면 된다 (한글·이모지 문구는 SSH grep에 쓰지 말 것).
#    다만 [9]단계의 Ollama 설치 스크립트가 컨테이너 상태에 따라 내부에서 apt를
#    부를 수 있어, 위의 DPkg::Lock::Timeout이 안전망으로 남아 있다.
...
if ! command -v node > /dev/null 2>&1; then
    curl -fsSL --retry 3 --retry-delay 2 https://deb.nodesource.com/setup_22.x | bash - > /dev/null 2>&1
...
echo "✅ APT 완료 — 이후 단계는 apt 미사용 (Claude Code 병렬 설치 시작 가능) [APT_DONE]"
...
wget -qO requirements_agent.txt \
...
if [ "${NOTOLAB_LOCK:-1}" = "1" ] && curl -fsSL --retry 3 --retry-delay 2 "$LOCK_URL" -o requirements-lock-agent.txt 2>/dev/null; then
    ...
    uv pip compile requirements_agent.txt -U -o requirements-lock-agent.txt -q
fi
uv pip install -r requirements-lock-agent.txt -q
