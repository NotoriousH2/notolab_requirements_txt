...
if [ "${NOTOLAB_FORCE:-0}" != "1" ] \
   ...
   && [ -x /tmp/.venv/bin/python ]; then
    ...
    echo "다시 설치하려면: NOTOLAB_FORCE=1 bash setup_peft.sh"
    exit 0
fi
...
if [ "${NOTOLAB_LOCK:-1}" = "1" ] && curl -fsSL --retry 3 --retry-delay 2 "$LOCK_URL" -o requirements-lock-PEFT.txt 2>/dev/null; then
    echo "  lock 사용: requirements-lock-PEFT.txt ($NOTOLAB_REF)"
    INSTALL_SOURCE=lock
else
    echo "  lock 없음 — manifest에서 해석"
    INSTALL_SOURCE=compile
    uv pip compile requirements_PEFT.txt \
        --index-strategy unsafe-best-match \
        --emit-index-url \
        -o requirements-lock-PEFT.txt -q
fi
uv pip install -r requirements-lock-PEFT.txt --index-strategy unsafe-best-match -q
