...
- 릴리스에 올라온 `setup_*.sh`에는 그 시점 버전이 박혀 있어, 실행하면 해당 시점의 `requirements_*.txt`를 받아 설치합니다.
- 어떤 버전이 적용됐는지는 설치 로그의 `[6/N] ... (버전: ...)` 줄과 마지막 완료 줄에서 확인합니다.
- 임시로 다른 버전을 쓰려면 `NOTOLAB_REF=<버전> bash setup_sllm.sh`

버전을 고정해도 완전한 재현은 아닙니다. 설치 시점에 의존성을 다시 해석하므로, 핀이 걸리지 않은 패키지는 시간이 지나면 다른 버전이 설치될 수 있습니다.

### 강사용: 새 버전 배포

변경을 `main`에 푸시한 뒤 실행합니다. 태그 생성, 버전이 박힌 스크립트 4개와 패키지 목록 4개 업로드까지 한 번에 처리합니다.
