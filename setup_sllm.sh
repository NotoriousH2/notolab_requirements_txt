...
if [ "${NOTOLAB_LOCK:-1}" = "1" ] && curl -fsSL --retry 3 --retry-delay 2 "$LOCK_URL" -o requirements-lock-sLLM.txt 2>/dev/null; then
    echo "  lock 사용: requirements-lock-sLLM.txt ($NOTOLAB_REF)"
    INSTALL_SOURCE=lock
else
    echo "  lock 없음 — manifest에서 해석"
    INSTALL_SOURCE=compile
    uv pip compile requirements_sLLM.txt \
        --index-strategy unsafe-best-match \
        --emit-index-url \
        -o requirements-lock-sLLM.txt -q
fi
uv pip install -r requirements-lock-sLLM.txt --index-strategy unsafe-best-match -q
